...
def train(config,
    ...
    eval_batch_step=[0,len(train_dataloader)]
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        for idx, batch in enumerate(train_dataloader):
            ...
            print_batch_step=len(train_dataloader)//10
            if(print_batch_step<2):print_batch_step=2
            # if dist.get_rank() == 0 and (
            #     # (global_step > 0 and global_step % print_batch_step == 0) or
            #     # (idx >= len(train_dataloader) - 1)):
            #     idx% print_batch_step==0):
            if dist.get_rank() == 0 and ((idx+1)% print_batch_step==0):
                ...
                #     total_samples / train_batch_cost, eta_sec_format)
                    #删除了norm_edit_dis
                strs = 'epoch:[{}/{}],step:[{}/{}], {},eta: {}'.format(
                    epoch, epoch_num, idx+1,all_step, logs,eta_sec_format)
                logger.info(strs)
            ...
            if global_step > start_eval_step and \
                ...
                for k, v in cur_metric.items():
                    if(k=='acc'):
                        cur_metric_str_list.append('{}: {:.6f}'.format(k, v))
                    else:
                        cur_metric_str_list.append('{}: {}'.format(k, v))
                cur_metric_str = 'cur metric, {}'.format(', '.join(cur_metric_str_list))
                ...
                if cur_metric[main_indicator] >= best_model_dict[
                    ...
                    best_model_dict[main_indicator] = cur_metric[main_indicator] - 0.005
                ...
                for k, v in best_model_dict.items():
                    if (k == 'acc'):
                        best_str_list.append('{}: {:.6f}'.format(k, v))
                    else:
                        best_str_list.append('{}: {}'.format(k, v))
                best_str = 'best metric, {}'.format(', '.join(best_str_list))
...
def preprocess(is_train,train_config):
    FLAGS = ArgsParser().parse_args()
    FLAGS.config=train_config
    ...
    if 'use_visualdl' in config['Global'] and config['Global']['use_visualdl']:
        ...
        vdl_writer_path = os.path.join(save_model_dir,'runs')
        if not os.path.exists(vdl_writer_path): os.makedirs(vdl_writer_path)
